...
public class FlatBufferBuilder @JvmOverloads constructor(
  ...
  public fun pad(byteSize: Int) {
    d("pad(space=$space, byteSize=$byteSize")
  ...
  public fun prep(size: Int, additionalBytes: Int) {
    ...
    while (space < alignSize + size + additionalBytes) {
      ...
      buffer.requestCapacity(buffer.capacity + alignSize + size + additionalBytes, copyAtEnd = true)
      space += buffer.capacity - oldBufSize
    }
    d("prep(space=$space, size=$size, additionalBytes=$additionalBytes")
    pad(alignSize)
  }
...
//    val copy: ReadWriteBuffer = buffer.writeSlice()
//    copy.limit(length)
//    return copy
//  }
 ...
 public fun createString(s: ReadBuffer): Int {
   ...
   * after this call.
   *
   * @param readBuffer A source [ReadBuffer] with data.
   * @return The offset in the buffer where the encoded array starts.
   */
 public fun createByteVector(data: ReadBuffer, from: Int = 0, until: Int = data.limit): Int {
  ...
  public fun startTable(numfields: Int) {
    ...
    nested = true
    objectStart = offset()
    d("startTable(numfields=$numfields, objectStart=$objectStart")
  }
  ...
  public fun required(table: Int, field: Int) {
    val table_start: Int = buffer.capacity - table
    val vtable_start: Int = table_start - buffer.getInt(table_start)
    val ok = buffer.getShort(vtable_start + field).toInt() != 0
   ...
   * `root_table`.
   */
  public fun finish(root_table: Int, file_identifier: String) {
 ...
 public fun sizedByteArray(start: Int = space, length: Int = buffer.capacity - space): ByteArray {
  ...
  public companion object {
     ...
     */
    public fun isFieldPresent(table: Table, offset: Int): Boolean = table.__offset(offset) != 0
  }
}
